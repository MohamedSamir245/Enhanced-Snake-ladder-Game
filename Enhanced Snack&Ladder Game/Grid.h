...
public:
	...
	void Gotostartcell(Player* player, const CellPosition& newPosition);
	...
	void DecreaseCurrentPlayer();    //used in Card_3 to make an extra roll

	Cell* Getcellfromcellposition(CellPosition);
	...
	Ladder * GetNextLadder(const CellPosition & position,CellPosition&);  // Gets a Pointer to the first Ladder after the passed "position"
	bool GetNextplayer(const CellPosition& position, Player**&); // Gets a Pointer to array of players pointer after the passed "position"...return true if player found
	bool HasCard(CellPosition cell); // used in copy card and cut card
	bool HasSnake(CellPosition cell);//check if the cell has snake
	bool HasLadder(CellPosition cell);//check if the cell has Ladder
	bool Isoverlapping(GameObject *newobj);
	Player* GetPoorestPlayer(); // Gets a Pointer to the poorest Player
	
	Card* GetGameObject(CellPosition); // used in copy card and cut card
	...
	void decreasewallets(Player*,int);//used in lightning attack
	...
	int getNumofLaddersSnakesCards(int& NumofSnakes, int& NumofCards);
