...
Output::Output()
{
	...
	UI.PlayerColors[0] = GREEN;
	UI.PlayerColors[1] = YELLOW;
	UI.PlayerColors[2] = RED;
...
int Output::GetCellStartX(const CellPosition & cellPos) const                      //done
{
...
int Output::GetCellStartY(const CellPosition & cellPos) const                     //done
{
	///TODO: implement the following function as described in Output.h file
...
void Output::DrawCardNumber(const CellPosition & cellPos, int cardNum) const      //done
{
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_EXIT1] = "images\\Menu_Exit.jpg";
	...
	MenuItemImages[ITM_Clear_Grid] = "images\\Menu_Clear_Grid.jpg";
...
void Output::PrintPlayersInfo(string info)//done
{
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~done
{
	// Get the Cell Number (from 1 to NumVerticalCells*NumHorizontalCells) and the X & Y of its upper left corner
	if (cardNum > 12)
	{
		PrintMessage("card number should be between 1 & 12");
		return;
	}
	...
	if (cardNum == 9)
		...
		pWind->SetBrush(DARKSALMON);
	else if (cardNum == 11)
		pWind->SetBrush(ORCHID);
	else if (cardNum == -1) // no card
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	{
		pWind->DrawLine(x12, toStartY + j * UI.CellHeight, x34, toStartY + j * UI.CellHeight, FRAME);
		j++;
	}
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const//done
{
	...
	int xv1 = x12,xv2=x12+xChange,xv3=xv1,xv4=x12-xChange;
	int yv1 = y1, yv2 = y1 + yChange,yv3=y1+2*yChange,yv4=yv2;
	int y[4] = { yv1,yv2,yv3,yv4 };
	int x[4] = { xv1,xv2,xv3,xv4 };
