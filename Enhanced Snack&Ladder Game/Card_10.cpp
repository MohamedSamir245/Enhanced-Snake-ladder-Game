...
void Card_10::ReadCardParameters(Grid* pGrid)
{
	...
	{
		//reading card price
		pOut->PrintMessage("New Card 10 : Enter its price amount ..."); // print to a descriptive message to the user
		...
		//reading card fees
		pOut->PrintMessage("New Card 10 : Enter its fees amount ..."); // print to a descriptive message to the user
		x = pIn->GetInteger(pOut);// Read an Integer from the user...need validations
		while (x <= 0) // validate x value
		{
			pOut->PrintMessage("invalid amount try again...");
...
void Card_10::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	if (owner == NULL)
	{
		pOut->PrintMessage("Enter (Y/y) to buy stations num : 10 Or otherwise to skip..."); // print to a descriptive message to the user
...
int Card_10::GetPrice()
{
	return price;
}

Player* Card_10::GetOwner()
{
	return owner;
}

void Card_10::SetOwner(Player* NewOwner)
{
	owner = NewOwner;
}
...
Card_10* Card_10::GetCopy()
{
	Card_10* pCard = new Card_10(position);
	pCard->fees = this->fees;
	pCard->price = this->price;
	return pCard;
}

bool Card_10::isSaved = false;
bool Card_10::isLoaded = false;